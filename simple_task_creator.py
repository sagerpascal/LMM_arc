...
def create_single_task_477d2879(image=None):
    def remove_single_pixels(data):
        for i in range(0, data.shape[0]):
            for j in range(0, data.shape[1]):
                if ((data[max(i-1, 0), j] != data[i, j] or i==0) and (data[min(i+1, data.shape[0]-1), j] != data[i, j] or i == data.shape[0]-1)) or ((data[i, max(j-1, 0)] != data[i, j] or j==0)  and (data[i, min(j+1, data.shape[1]-1)] != data[i, j] or j == data.shape[1]-1)):
                    data[i, j] = 0
        return data
    ...
    def mark_border(data):
        ...
        for i in range(0, data.shape[0]):
            for j in range(0, data.shape[1]):
                if data[i, j] != 0:
                    if data[max(i-1, 0), j] != data[i, j] or data[min(i+1, data.shape[0]-1), j] != data[i, j] or data[i, max(j-1, 0)] != data[i, j] or data[i, min(j+1, data.shape[0]-1)] != data[i, j] or data[max(i-1, 0), max(j-1, 0)] != data[i, j] or data[max(i-1, 0), min(j+1, data.shape[0]-1)] != data[i, j] or data[min(i+1, data.shape[0]-1), max(j-1, 0)] != data[i, j] or data[min(i+1, data.shape[0]-1), min(j+1, data.shape[0]-1)] != data[i, j]:
                        borders[i, j] = 1
        return borders

    def color_image(connected_components_no_border, connected_components, border):
        values = np.unique(connected_components_no_border)
        colors = random.sample(range(2, 20), len(values)*2)
        result = np.zeros(connected_components_no_border.shape, dtype=int)
        for i, v in enumerate(values):
            result[connected_components_no_border == v] = colors[i*2]
            result[(connected_components == v) & (border == 1)] = colors[i * 2 + 1]
        #result[border == 1] = 1
        return result
    ...
    def check_if_ok(borders):
        ...
        for i in range(1, borders.shape[0]-1):
            for j in range(1, borders.shape[1]-1):
                if (borders[i-1, j] == color_bg or i-1 == 0) and (borders[i+1, j] == color_bg or i+1 == borders.shape[0]) and (borders[i, j-1] == color_bg or j-1==0) and (borders[i, j+1] == color_bg or j+1 == borders.shape[1]):
                    return False
        return True
    ...
    # Hack to fix problem with components that share a border (remove border and relabel)
    connected_components_no_border = skimage.measure.label(1 - borders, connectivity=1)
    # connected_components_no_border = connected_components.copy()
    # connected_components_no_border[borders == 1] = 0
    # connected_components_no_border = skimage.measure.label(connected_components_no_border, connectivity=2)
    # connected_components_no_border[borders == 1] = 1

    # connected_components_no_border is needed to get different colors for different borders
    result = color_image(connected_components_no_border, connected_components, borders)
